...
import { ReportStudentDetail } from '@/models/api'
...
const ReportStudentWrapper = () => {
  ...
  useState(false)
  const [totalReportStudent, setTotalReportStudent] = useState(0)

  const [ReportStudentChoose, setReportStudentChoose] =
    useState<ReportStudentDetail[]>()
  const filterReportStudent = useFilterForReportStudentStore()
  const allReportStudent = useGetAllReportStudent()
  const tableRef = useRef<any>()
  useEffect(() => {
    if (allReportStudent.status === 'success')
      setTotalReportStudent(allReportStudent.data.total)
  }, [allReportStudent])
  return (
    <>
      <div className="">
              ...
              <DebouncedInput
                ...
                onChange={(value) => {
                  filterReportStudent.update(
                    produce(filterReportStudent.filter, (draftState) => {
        ...
        {allReportStudent.status === 'error' && <PageError />}
        {allReportStudent.status === 'loading' && <TableSkeleton />}
        {allReportStudent.status === 'success' && (
          <ReportTable
            ref={tableRef}
            getAllReportStudentData={allReportStudent.data}
            setReportStudentChoose={setReportStudentChoose}
            isPreviousData={allReportStudent.isPreviousData}
          />
        )}
      </div>
    </>
