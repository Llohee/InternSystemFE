...
import { useGetAccountHumanresourceById } from '@/hooks/query/account/humanresource'
import { UserGetDetail } from '@/models/api'
import { useState } from 'react'
import { FormHumanresourceAccount } from './form-humanresourceaccount'
import { useHumanresourceUpdate } from './hook'

const UpdateHumanresourceModal = (props: {
  isOpen: boolean
  closeModal: () => void
  humanresourceDetail: UserGetDetail
  type: string
}) => {
  const getHumanresourceById = useGetAccountHumanresourceById(props.humanresourceDetail.id)
  if (getHumanresourceById.status === 'loading')
    return (
      <ModalLoading
        length={5}
        size="xl"
  ...
  if (getHumanresourceById.status === 'error') return <></>
  return (
    <>
      <UpdateModal
        closeModal={props.closeModal}
        humanresourceDetail={getHumanresourceById.data}
        isOpen={props.isOpen}
        type={props.type}
      />
    </>
...
const UpdateModal = (props: {
  ...
  const { handleFormSubmit, formUpdate, mutation } = useHumanresourceUpdate(
    props.closeModal,
    props.humanresourceDetail
  )
      ...
      >
        <div className="relative">
          <FormHumanresourceAccount
            ...
            humanresourceDetail={props.humanresourceDetail}
            type={props.type}
            isEdit={true}
          />
        </div>
...
export default UpdateHumanresourceModal
