...
import { useGetAllAccountHumanresource } from '@/hooks/query/account/humanresource'
...
import AccountHumanresourceTable from './table'
...
const HumanresourceAccount = () => {
  ...
  const [humanresourceAccountChoose, setHumanresourceAccountChoose] =
    useState<UserGetDetail[]>()
  const [isShowModalUpdateAction, setIsShowModalUpdateAction] = useState(false)
  const filterHumanresourceAccount = useFilterForHumanresourceAccountStore()
  // const templateLink = useGetTemplateLink()
  const allAccountHumanresource = useGetAllAccountHumanresource()
  const tableRef = useRef<any>()
  useEffect(() => {
    if (allAccountHumanresource.status === 'success')
      setTotalAccount(allAccountHumanresource.data.total)
  }, [allAccountHumanresource])
  return (
    <>
      <div className="">
          ...
          <div className="flex gap-5 grow items-center">
            <div className="ml-2">
              <DebouncedInput
                ...
                value={filterHumanresourceAccount.filter.name}
                className="lg:w-96 "
                onChange={(value) => {
                  filterHumanresourceAccount.update(
                    produce(filterHumanresourceAccount.filter, (draftState) => {
        ...
        {allAccountHumanresource.status === 'error' && <PageError />}
        {allAccountHumanresource.status === 'loading' && <TableSkeleton />}
        {allAccountHumanresource.status === 'success' && (
          <AccountHumanresourceTable
            ref={tableRef}
...
export default HumanresourceAccount
