...
import { useGetAllAccountLecturer } from '@/hooks/query/account/lecturer'
...
import AccountLecturerTable from './table'
import { DebouncedInput } from '@/components/ui/input/debouced-input'
import produce from 'immer'
import { SearchIcon } from '@/components/ui/icon'
import CreateLecturerAccountModal from './modal/create-lecturer'
...
const LecturerAccount = () => {
  ...
  const [LecturerAccountChoose, setLecturerAccountChoose] =
    useState<UserGetDetail[]>()
  const [isShowModalUpdateAction, setIsShowModalUpdateAction] = useState(false)
  const filterLecturerAccount = useFilterForLecturerAccountStore()
  // const templateLink = useGetTemplateLink()
  const allAccountLecturer = useGetAllAccountLecturer()
  const tableRef = useRef<any>()
  useEffect(() => {
    if (allAccountLecturer.status === 'success')
      setTotalAccount(allAccountLecturer.data.total)
  }, [allAccountLecturer])
  return (
    <>
      <div className="">
          ...
          <div className="flex gap-5 grow items-center">
            <div className="ml-2">
              <DebouncedInput
                ...
                value={filterLecturerAccount.filter.name}
                className="lg:w-96 "
                onChange={(value) => {
                  filterLecturerAccount.update(
                    produce(filterLecturerAccount.filter, (draftState) => {
        ...
        {allAccountLecturer.status === 'error' && <PageError />}
        {allAccountLecturer.status === 'loading' && <TableSkeleton />}
        {allAccountLecturer.status === 'success' && (
          <AccountLecturerTable
            ref={tableRef}
            getAllAccountLecturerData={allAccountLecturer.data}
            setLecturerAccountChoose={setLecturerAccountChoose}
            isPreviousData={allAccountLecturer.isPreviousData}
          />
        )}
      </div>
      <CreateLecturerAccountModal
