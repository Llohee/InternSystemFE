...
import ConfirmDeleteLecturerModal from '../modal/confirm-delete-lecturer'
import UpdateLecturerModal from '../modal/update-lecturer'

interface LecturerAccountsProps {
  getAllAccountLecturerData: GetAllUsersResponse
  setLecturerAccountChoose: (lectureraccount: UserGetDetail[]) => void
  isPreviousData: boolean
}

const AccountLecturerTable = (props: LecturerAccountsProps, ref: any) => {
  const filterUser = useFilterForLecturerAccountStore()
  const [data, setData] = useState(() => [
    ...props.getAllAccountLecturerData.data,
  ])
  ...
  const [lecturerChoose, setLecturerChoose] = useState<UserGetDetail>()
  const {
    toggleChooseAllItem,
    toggleChooseItem,
    setDataChoose,
  ...
  useEffect(() => {
    props.setLecturerAccountChoose(itemChoose)
  }, [itemChoose])
  useEffect(() => {
    if (isShowModalUpdate) {
      props.setLecturerAccountChoose([])
      clearChooseItems()
    }
  }, [isShowModalUpdate])
  ...
  useEffect(() => {
    setData([...props.getAllAccountLecturerData.data])
    setDataChoose(props.getAllAccountLecturerData.data)
    clearChooseItems()
  }, [props.getAllAccountLecturerData])
        ...
        <div className="flex gap-3">
          <Button
            ...
            onClick={() => {
              ...
              setLecturerChoose(info.row.original)
            }}
          >
            <svg
              width="17"
    ...
    <>
      ...
      <div className="flex gap-2 py-4 border-t border-border-2 justify-end">
        <Pagination
          ...
          pageCurrent={props.getAllAccountLecturerData.page + 1}
          totalPage={props.getAllAccountLecturerData.total_page}
          label={
            props.getAllAccountLecturerData.total > 0 ? (
              <div className="hidden md:block">
                {props.getAllAccountLecturerData.page *
                ...
                {props.getAllAccountLecturerData.page *
                  filterUser.filter.limit +
                  data.length}{' '}
                trên tổng {props.getAllAccountLecturerData.total}
              </div>
            ) : (
              <></>
            )
      ...
      {lecturerChoose && isShowModalDelete && (
        <ConfirmDeleteLecturerModal
          isOpen={isShowModalDelete}
          closeModal={() => {
            setIsShowModalDelete(false)
            setLecturerChoose(undefined)
          }}
          lecturerDetail={lecturerChoose}
        />
      )}
      {lecturerChoose && isShowModalUpdate && (
        <UpdateLecturerModal
...
export default forwardRef(AccountLecturerTable)
