...
import { useGetLecturerById } from '@/hooks/query/account/lecturer'
import { UserGetDetail } from '@/models/api'
import { useState } from 'react'
import { FormLecturerAccount } from './form-lectureraccount'
import { useLecturerUpdate } from './hook'

const UpdateLecturerModal = (props: {
  isOpen: boolean
  closeModal: () => void
  lecturerDetail: UserGetDetail
}) => {
  const getLecturerById = useGetLecturerById(props.lecturerDetail.id)
  if (getLecturerById.status === 'loading')
    return (
      <ModalLoading
        length={5}
        size="default"
  ...
  if (getLecturerById.status === 'error') return <></>
  return (
    <>
      <UpdateModal
        closeModal={props.closeModal}
        lecturerDetail={getLecturerById.data}
        isOpen={props.isOpen}
      />
    </>
  )
...
const UpdateModal = (props: {
  ...
  const { handleFormSubmit, formUpdate, mutation } = useLecturerUpdate(
    props.closeModal,
    props.lecturerDetail
  )
      ...
      >
        <div className="relative">
          <FormLecturerAccount
            ...
            lecturerDetail={props.lecturerDetail}
            isEdit={true}
          />
        </div>
        <ConfirmCloseModal
