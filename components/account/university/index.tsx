...
import { UniversityAccountGetDetail } from '@/models/api'
...
const UniversityAccount = () => {
  ...
  const [usersChoose, setUserChoose] = useState<UniversityAccountGetDetail[]>()
  ...
  useEffect(() => {
    if (allAccountUniversity.status === 'success')
      setTotalUser(allAccountUniversity.data.total)
  ...
  return (
    <>
        ...
        <div className="flex items-center py-4">
          <div className="flex gap-5 grow items-center">
            <div className="ml-2">
              <DebouncedInput
                ...
                value={filterUser.filter.name}
                className="lg:w-96 "
                onChange={(value) => {
                  filterUser.update(
                    produce(filterUser.filter, (draftState) => {
          ...
          <div className="flex flex-col gap-3 md:hidden fixed z-10 bottom-14 right-2">
            ...
            <Button
              ...
              size="small"
              iconOnly
              ariaLabel="Open create user modal"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
      ...
      />
      <UpdateActivationModal
        active={changeActive}
        users={usersChoose}
        isOpen={isShowModalUpdateAction}
        closeModal={() => {
          tableRef.current.clearChooseItems()
          setIsShowModalUpdateAction(false)
        }}
      />
      <ActionButtonsSelected
        itemChoose={usersChoose}
        onChangeActive={(b) => {
          setIsShowModalUpdateAction(true)
          setChangeActive(b)
        }}
      /> */}
    </>
