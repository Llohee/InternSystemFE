...
import {
  GetAllUniversityAccountResponse,
  UniversityAccountGetDetail,
} from '@/models/api'
import {
  ColumnFiltersState,
  SortingState,
...
interface UsersProps {
  getAllAccountUniversityData: GetAllUniversityAccountResponse

  setUsersChoosse: (users: UniversityAccountGetDetail[]) => void
  isPreviousData: boolean
}

const AccountUniversityTable = (props: UsersProps, ref: any) => {
  ...
  const [userChoose, setUserChoose] = useState<UniversityAccountGetDetail>()
  const {
    toggleChooseAllItem,
    toggleChooseItem,
    setDataChoose,
    ...
    chooseAllItems,
    itemChoose,
  } = useChooseMulti<UniversityAccountGetDetail>({ data: data })
  useEffect(() => {
    filterUser.update(
  ...
  useEffect(() => {
    props.setUsersChoosse(itemChoose)
  }, [itemChoose])
  useEffect(() => {
    if (isShowModalUpdate) {
      props.setUsersChoosse([])
      clearChooseItems()
    }
  }, [isShowModalUpdate])
  ...
  const columnHelper = createColumnHelper<UniversityAccountGetDetail>()
