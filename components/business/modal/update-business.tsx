...
import { useBusinessUpdate } from './hook'
...
const UpdateBusinessModal = (props: UpdateBusiness) => {
  const getBusinessById = useGetTenantById(
    props.businessDetail.id,
    props.type
  )
  if (getBusinessById.status === 'loading')
    return (
      <ModalLoading
        length={5}
        size="xl"
  ...
  if (getBusinessById.status === 'error') return <></>
  return (
    <>
      <UpdateModal
        closeModal={props.closeModal}
        businessDetail={getBusinessById.data}
        type={props.type}
        isOpen={props.isOpen}
      />
    </>
...
const UpdateModal = (props: {
  ...
  const { handleFormSubmit, formUpdate, mutation } = useBusinessUpdate(
    props.closeModal,
