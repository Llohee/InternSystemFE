...
const ReportLecturerWrapper = (props: { profession: string }) => {
  ...
  useEffect(() => {
    if (allReportLecturer.status === 'success')
      setTotalReportLecturer(allReportLecturer.data.total)
  }, [allReportLecturer])
  return (
    <>
      <div className="">
              ...
              <DebouncedInput
                ...
                onChange={(value) => {
                  filterReportLecturer.update(
                    produce(filterReportLecturer.filter, (draftState) => {
        ...
        {allReportLecturer.status === 'error' && <PageError />}
        {allReportLecturer.status === 'loading' && <TableSkeleton />}
