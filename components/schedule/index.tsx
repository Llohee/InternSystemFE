...
import { useGetAllSchedule } from '@/hooks/query/schedule'
...
import CreateScheduleModal from './modal/create-schedule'
...
const ScheduleWrapper = () => {
  ...
  const allSchedule = useGetAllSchedule()
  const tableRef = useRef<any>()
  useEffect(() => {
    if (allSchedule.status === 'success')
      setTotalSchedule(allSchedule.data.total)
  }, [allSchedule])
  return (
    <>
      <div className="">
          ...
          <div className="flex gap-5 grow items-center">
            <div className="ml-2">
              <DebouncedInput
                ...
                value={filterSchedule.filter.name}
                className="lg:w-96 "
                onChange={(value) => {
                  filterSchedule.update(
                    produce(filterSchedule.filter, (draftState) => {
        ...
        {allSchedule.status === 'error' && <PageError />}
        {allSchedule.status === 'loading' && <TableSkeleton />}
        {allSchedule.status === 'success' && (
          <ScheduleTable
            ref={tableRef}
            getAllScheduleData={allSchedule.data}
            setScheduleChoose={setScheduleChoose}
            isPreviousData={allSchedule.isPreviousData}
          />
        )}
      </div>
      <CreateScheduleModal
